...
getRandomUsers <- function(data, nUsers, seed) {
  set.seed(seed)
  testUsers <- sample(1:nrow(data), nUsers)
...
```{r}  
testUsers <- getRandomUsers(MovieLense, 20, 42)
testData <- getRandomDataset(MovieLense, 20, 42)
testUsers
testData
```
...
We selected 20 random users from both reduced datasets.

2. Bestimme den Anteil der Top-N Empfehlung nach Genres pro Nutzer*in,

```{r}
svd7 <- Recommender(testData, "SVD", param = list(k = 7))
ubcf13 <- Recommender(testData, "UBCF", parameter = list(method = "Euclidean", nn = 13))
ibcf22 <- Recommender(testData, "IBCF", param = list(k = 22, method = "cosine"))
svd50 <- Recommender(testData, "SVD", param = list(k = 5))
```
