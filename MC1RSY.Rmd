...
```{r}
svd30 <- Recommender(Data, "SVD", param = list(k = 30))
...
svd50 <- Recommender(Data, "SVD", param = list(k = 5))
```
...
topN_svd7 <- predict(svd7, Data, n = N)
topN_ubcf13 <- predict(ubcf13, Data, n = N)
topN_ibcf22 <- predict(ibcf22, Data, n = N)
topN_svd50 <- predict(svd50, Data, n = N)
```

```{r}
getTopNRec <- function(users, dataset, model) {
  topN_model <- predict(model, dataset, n = 10)
  topN_svd7_list <- as(topN_svd7, "list")
  topN_svd7_tibble <- as_tibble(topN_svd7_list)
  topN_svd7_dataFrame <- as.data.frame(topN_svd7_tibble)
  colnames(topN_svd7_dataFrame) <- Users
  topN_svd7_dataFrame_T <- t(topN_svd7_dataFrame)
  topN_svd7_dataFrame_T_tibble <- as_tibble(topN_svd7_dataFrame_T)
  topN_svd7_dataFrame_T_tibble$Users <- Users
  topN_svd7_dataFrame_T_tibble_long <- pivot_longer(
    topN_svd7_dataFrame_T_tibble, cols = 1:10, names_to = "topN", values_to = "itemID")
  topN_svd7_dataFrame_T_tibble_long_genre <- left_join(
    topN_svd7_dataFrame_T_tibble_long, MovieLenseMeta, by = c("itemID" = "title"))
  topNTibble <- select(
    topN_svd7_dataFrame_T_tibble_long_genre, -topN, -year, -url, -itemID)
  topNRec <- topNTibble %>% group_by(Users) %>%
    summarise(across(everything(), ~ sum(., is.na(.), 0)))
}
topNRec <- getTopNRec(Users, Data, svd7)
topNRec
```

```{r}
resultsList <- list()
modelsList <- list(svd7, ubcf13, ibcf22, svd50)
