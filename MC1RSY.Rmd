...
```{r}
ibcfTopNList2 <- predict(ibcf, testSet, n = 15)
ibcfTopNList2

ubcfTopNList2 <- predict(ubcf, testSet, n = 15)
ubcfTopNList2
```
...
```
First Reduction Set:
IBCF recommends 380 movies.
UBCF recommends 218 movies.
The intersect between both recommenders for the first reduced dataset is 119 movies.
The relative intersect for IBCF is 31% and for UBCF it is 55%.
...
# test that the ratings have mean 0 and sd 1
...
```
IBCF with normalized data recommends 366 movies.
UBCF with normalized data recommends 211 movies.
The intersect between both recommenders for the first reduced dataset (normalized) is 117 movies.
The relative intersect for IBCF is 33% and for UBCF it is 58%.
The results are similar as without normalized ratings.
But we get a small increase in similar recommendations, due to normalizing.
Normalized ratings make the recommendations more similar.
WHY?
